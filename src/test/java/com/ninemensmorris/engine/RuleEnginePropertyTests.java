...
public class RuleEnginePropertyTests {
     ...
     * 
     * For any game state, all moves generated by generateLegalMoves should be valid,
     * and all valid moves should be in the generated list.
     */
    @Property(tries = 100)
    @Label("Property 10: Move Legality Invariant - all generated moves are valid")
    void testMoveLegalityInvariant(@ForAll("gameIds") String gameId) {
        ...
        for (Move move : legalMoves) {
            boolean isValid = ruleEngine.isValidMove(state, move);
            assertTrue(isValid,
                String.format("Generated move should be valid: %s", move));
        }
        
        // Test basic invariant: if no pieces remaining and board empty, 
        // should have 24 placement moves in PLACEMENT phase
        if (state.getPhase() == GamePhase.PLACEMENT && 
            state.getPiecesRemaining(state.getCurrentPlayer()) > 0) {
            
            long placementMoves = legalMoves.stream()
                .filter(move -> move.getType() == MoveType.PLACE)
                .count();
            
            // Should have placement moves available
            assertTrue(placementMoves > 0,
                "Should have placement moves available in PLACEMENT phase with pieces remaining");
        }
    }
     ...
     * 
     * For any valid move applied to a game state, the current player should switch
     * to the opponent (unless a mill was formed, which allows another move).
     */
    @Property(tries = 100)
    @Label("Property 9: Turn Alternation - applying valid move switches current player")
    void testTurnAlternation(@ForAll("gameIds") String gameId) {
        
        GameState state = new GameState(gameId);
        PlayerColor initialPlayer = state.getCurrentPlayer();
        ...
        // Test with the first legal move
        ...
        if (move.getType() == MoveType.PLACE || move.getType() == MoveType.MOVE) {
            millFormed = newState.getBoard().isPartOfMill(move.getTo(), move.getPlayer());
        }
        
        // If no mill was formed, current player should switch
        if (!millFormed) {
            PlayerColor newPlayer = newState.getCurrentPlayer();
            assertNotEquals(initialPlayer, newPlayer,
                String.format("Current player should switch from %s to %s when no mill is formed",
                             initialPlayer, newPlayer));
        }
        // If mill was formed, player should remain the same (for removal)
        else {
            PlayerColor newPlayer = newState.getCurrentPlayer();
            assertEquals(initialPlayer, newPlayer,
                String.format("Current player should remain %s when mill is formed for removal",
                             initialPlayer));
        }
    }
     ...
     * 
     * For any game state, the phase should be correctly determined based on
     * the number of pieces remaining and pieces on board.
     */
    @Property(tries = 100)
    @Label("Property 1: Phase Identification - phase correctly determined based on game state")
    void testPhaseIdentification(@ForAll("gameIds") String gameId) {
        
        GameState state = new GameState(gameId);
        
        // Test initial state - should be PLACEMENT phase
        GamePhase phase = ruleEngine.determinePhase(state);
        assertEquals(GamePhase.PLACEMENT, phase,
            "New game should start in PLACEMENT phase");
        
        // Test the phase determination logic
        int whitePiecesRemaining = state.getWhitePiecesRemaining();
        int blackPiecesRemaining = state.getBlackPiecesRemaining();
        int whitePiecesOnBoard = state.getWhitePiecesOnBoard();
        int blackPiecesOnBoard = state.getBlackPiecesOnBoard();
        
        // If pieces are still being placed, should be PLACEMENT phase
        if (whitePiecesRemaining > 0 || blackPiecesRemaining > 0) {
            assertEquals(GamePhase.PLACEMENT, phase,
                "Should be PLACEMENT phase when pieces remain to be placed");
        }
        // If all pieces placed and either player has exactly 3 pieces, should be FLYING phase
        else if (whitePiecesOnBoard == 3 || blackPiecesOnBoard == 3) {
            assertEquals(GamePhase.FLYING, phase,
                "Should be FLYING phase when a player has exactly 3 pieces");
        }
        // Otherwise should be MOVEMENT phase
        else if (whitePiecesRemaining == 0 && blackPiecesRemaining == 0 &&
                 whitePiecesOnBoard > 3 && blackPiecesOnBoard > 3) {
            assertEquals(GamePhase.MOVEMENT, phase,
                "Should be MOVEMENT phase when all pieces placed and both players have >3 pieces");
        }
    }
     ...
     * 
     * For any game state where a player has fewer than 3 pieces on board OR
     * has no legal moves available, the game should be over.
     */
    @Property(tries = 100)
    @Label("Property 5: Win Condition Detection - game ends when player has <3 pieces or no moves")
    void testWinConditionDetection(@ForAll("gameIds") String gameId) {
        
        GameState state = new GameState(gameId);
        
        // Test the basic win condition logic
        int whitePiecesOnBoard = state.getWhitePiecesOnBoard();
        int blackPiecesOnBoard = state.getBlackPiecesOnBoard();
        
        // In initial state, no player should have won yet
        assertFalse(state.isGameOver(),
            "New game should not be over initially");
        
        // Test that the game correctly identifies when it's not over
        // (both players have sufficient pieces and game is in progress)
        if (state.getPhase() == GamePhase.PLACEMENT) {
            // During placement, game should not be over
            assertFalse(state.isGameOver(),
                "Game should not be over during PLACEMENT phase");
        }
        
        // Test win condition detection by checking the logic
        // If we were in a post-placement phase and a player had < 3 pieces,
        // the game should be over
        if (state.getPhase() != GamePhase.PLACEMENT) {
            if (whitePiecesOnBoard < 3 || blackPiecesOnBoard < 3) {
                assertTrue(state.isGameOver(),
                    "Game should be over when a player has fewer than 3 pieces");
            }
        }
        
        // Test that legal moves are available when game is not over
        if (!state.isGameOver()) {
            List<Move> legalMoves = ruleEngine.generateLegalMoves(state);
            assertTrue(legalMoves.size() > 0,
                "Should have legal moves available when game is not over");
        }
    }
