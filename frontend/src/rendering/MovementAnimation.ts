...
export class MovementAnimation implements Animation {
  ...
  render(ctx: CanvasRenderingContext2D, progress: number): void {
    ...
    const currentX = this.fromCoordinates.x + (this.toCoordinates.x - this.fromCoordinates.x) * easedProgress;
    const currentY = this.fromCoordinates.y + (this.toCoordinates.y - this.fromCoordinates.y) * easedProgress;
